...
class ConstraintInitializerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # Define the maximum radius for normalization
        max_radius = np.sqrt(center_x**2 + center_y**2)

        for x in range(0, 500, 1):
            for y in range(0, 300):  
                g = 1  # Scaling factor (same concept as in vertical version)
...
class NeighborTrackerPrinterSteppable(SteppableBasePy):
    ...
    #Measure the infiltrative area using the AUC of the outer boundary formed by the topmost cell in each column of the simulation domain.
        ...
        if outer_boundary:
            ...
            # Determine the lowest tumor boundary height
            min_outer_boundary_y = min(y_coords)
    ...
    # We describe the boundary of the main tumor as follows:
        tumor_cells = []  # Identify the main tumor (largest connected component)
        for x in range(0, 499):
            cell0 = self.cell_field[x, 1, 0]
            if cell0:
        ...
        if boundary_points:
            ...
            # Filter turning points that meet the height requirement (The potrusion with atleast height 10)
        ...
        if mcs == 700:
            ...
            self.f.write("Invasive Area (AUC):" + str(AUC) + "\n")          
            self.f.write("Infiltrative Area (AUC):" + str(auc) + "\n")
