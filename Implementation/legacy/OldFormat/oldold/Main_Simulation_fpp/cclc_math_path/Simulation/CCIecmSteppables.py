...
class ConstraintInitializerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        print("LC:" + str(len(self.cell_list_by_type(self.LC))) + "\n")
        print("FC:" + str(len(self.cell_list_by_type(self.FC))) + "\n")
    ...
    def step(self, mcs): 
        ...
        if mcs % 100 == 0:
            xmax, ymax = 500, 300  
            y_initial = 21  # Initial slab's top boundary
            outer_boundary = []  # Store the outer boundary coordinates

            # Iterate through each column to find the topmost non-medium cell beyond the initial layout
            for x in range(0, xmax):
                y = ymax - 1  # Start from the top of the column
                while y > y_initial and (not self.cell_field[x, y, 0] or self.cell_field[x, y, 0].type == self.MEDIUM):
                    y -= 1  # Move downward while encountering medium or no cells

                # Record the coordinates of the first non-medium cell found above y_initial
                if y > y_initial:
            ...
            if outer_boundary:
                ...
                self.plot_win5.add_data_point("Outer_Boundary", x_coords, y_coords)

                # Calculate the infiltrative area (AUC) using the trapezoidal rule
                auc = np.trapz(np.array(y_coords) - y_initial, x_coords)  # Subtract y_initial to exclude the initial slab
            else:
                auc = 0
                
            self.m.write(f"MCS {mcs}: Infiltrative Area (AUC): {auc}\n")
...
class NeighborTrackerPrinterSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        total_cells_array = []
        # Log cluster composition at each step
      ...
      #      self.f.write("Stalks: " + str(stalks) + "\n")
            #self.f.write("Average Space Between Stalks: " + str(avgspace) + "\n")
       #     self.f.write("Average Height of Stalks: " + str(avgheight) + "\n")
        #    self.f.write("Variance in Height of Stalks: " + str(heightvar) + "\n")
            self.f.write("MAX: " + str(max) +"\nMIN: " + str(min) + "\n")
            self.f.write("Stalk Leaders: " + str(stalklc) +"\n")
            self.f.write("CLUSTER DATA:\nClusters: " + str(clusters) +"\n")
            #self.f.write("Cluster Leaders: " + str(len(clusterlc)) +"\n")
            #self.f.write("Cluster Followers: " + str(clusterfc) +"\n")
            self.f.write("Invasive Area: " + str(invarea) +"\n")
