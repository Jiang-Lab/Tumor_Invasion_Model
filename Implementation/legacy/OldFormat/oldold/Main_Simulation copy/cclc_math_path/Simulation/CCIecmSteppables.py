...
class NeighborTrackerPrinterSteppable(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        epheight = [self.fetch_cell_by_id(node).yCOM - min for node in epnodes if self.fetch_cell_by_id(node)]
        ...
        if epheight:
            ...
            branch_threshold = avgheight + 10
            
            # Count the number of branches (stalks exceeding threshold)
            branches = sum(1 for h in epheight if h > branch_threshold)
        else:
            avgheight = 0
            heightvar = 0
            maxheight = 0
       ...
       # stalks = len(endpoints)
        ...
        if mcs % 100 == 0:
            ...
            if outer_boundary:
                ...
                auc = np.trapz(y_coords, x_coords) - (min * 500)
            else:
                auc = 0
        ...
        if mcs == 700:
            ...
            self.f.write("Total Defected Cells (Singles+Clusters): " + str(defects) + "\n")
            self.f.write("Stalks: " + str(stalks) + "\n")
