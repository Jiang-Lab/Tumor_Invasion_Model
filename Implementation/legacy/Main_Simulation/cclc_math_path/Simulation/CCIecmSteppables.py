        ...
        for cell in self.cell_list_by_type(self.FC):
            if cell.id not in Tumorcells and cell.id not in cluster_checked:
                ...
                if len(new_cluster_cells) >= 2:
                    ...
                    leader_xs, leader_ys = [], []
                    follower_xs, follower_ys = [], []

                    for cid in new_cluster_cells:
                        c = self.fetch_cell_by_id(cid)
                        if c:
                            if c.type == self.LC:
                                leader_xs.append(c.xCOM)
                                leader_ys.append(c.yCOM)
                            elif c.type == self.FC:
                                follower_xs.append(c.xCOM)
                                follower_ys.append(c.yCOM)
...
#==============================================================================================================================   
        if mcs % 100 == 0:
            ...
            boundary_data = [[x_array[j], main_tumor_array[j], outermost_array[j], lowest_point_array[j]] for j in range(len(x_array))]

            defector_positions = []
            for cell_id in detached_cells:
                cell = self.fetch_cell_by_id(cell_id)
                if cell:
                    defector_positions.append([cell.id, cell.xCOM, cell.yCOM])
...
#==============================================================================================================================   
    ...
    def save_cluster_compositions(self, clusters_data, mcs):
        ...
        with open(cluster_filename, "w", newline="") as cluster_file:
            ...
            cluster_writer.writerow([
                "Cluster ID", "Leader Cells", "Follower Cells", "Total Cells",
            ...
            for cluster in clusters_data:
                cluster_writer.writerow([
                    cluster["cluster_id"], cluster["leader_cells"], cluster["follower_cells"], cluster["total_cells"],
    ...
    def save_mcs_data(self, output_dir1, mcs, 
        ...
        save_csv("BoundaryData",
                 ["X", "Main_Tumor", "Outermost", "Lowest_boundary_point"],
                 boundary_data)

        save_csv("DefectorPosition",
                 ["id", "x", "y"],
                 defector_positions)

        save_csv("TumorPosition",
                 ["id", "x", "y"],
                 tumor_positions)

        save_csv("TumorLeaderCells",
                 ["id", "x", "y"],
                 tumor_leader_cells)

        save_csv("TumorFollowerCells",
                 ["id", "x", "y"],
                 tumor_follower_cells)
