...
class NeighborTrackerPrinterSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.ClusterComposition_writer.writerow([
            "Cluster ID", "Leader Cells", "Follower Cells", "Total Cells", "Centroid_X", "Centroid_Y",
                        ...
                        for cid in new_cluster_cells:
                            ...
                            if cell:
                                if cell.type == self.LC:
                                    leader_xs.append(cell.xCOM)
                                    leader_ys.append(cell.yCOM)
                                elif cell.type == self.FC:
                                    follower_xs.append(cell.xCOM)
                                    follower_ys.append(cell.yCOM)

                        self.ClusterComposition_writer.writerow([
                            clusters, num_leader_cells, num_follower_cells, len(new_cluster_cells),
                            centroid_x, centroid_y,
                            str(leader_xs), str(leader_ys),
                            str(follower_xs), str(follower_ys)
                        ])
